...
- `agentbox_operations_start_next`：将下一个计划操作切换为 `currentOperation`。
- `agentbox_operations_next_action`：读取 `currentOperation` 中下一个待执行或可执行 action。
- `agentbox_operations_update_action`：手动更新某个 action 的状态或执行元数据。
- `agentbox_operations_finish_current`：将已完成的当前操作归档。
- `agentbox_operations_cancel_current`：取消当前操作，并停止继续执行剩余 action。
- `agentbox_operations_clear_completed`：清理已完成操作历史。
- `agentbox_operations_reconcile`：对比本地 operation state 与链上状态，并保守修复明显冲突。
...
- `agentbox_skills_check_gather_prerequisites`：检查采集条件。
...
耗时链上动作：交易成功后只表示动作已开始，角色会进入对应状态；必须等待条件满足后再调用 `agentbox_skills_finish_current_action` 才算完整结束。

- `agentbox_skills_teleport_start`：开始传送；要求 `Idle`、目标在地图内且不是当前位置；预计区块数为 `ceil(距离 / speed)`。
- `agentbox_skills_gather_start`：开始采集；要求 `Idle`、站在资源点、已学习对应技能、资源点未满；预计区块数为 `amount`。
- `agentbox_skills_learn_npc_start`：向 NPC 学习；要求 `Idle`、位于 NPC 精确坐标、NPC 空闲、技能未学会；预计区块数为该技能的 `skillRequiredBlocks`。
- `agentbox_skills_learn_player_request`：向玩家发起学习请求；要求双方同坐标，teacher 已学会目标技能；本步骤进入等待 teacher 接受的状态，不开始区块倒计时；teacher 接受后预计区块数为 `skillRequiredBlocks * 2`。
- `agentbox_skills_learn_player_accept`：接受玩家学习；要求 teacher `Idle`，student 正在等待该 teacher 接受；预计区块数为 student 请求中记录的 `skillRequiredBlocks * 2`。
- `agentbox_skills_craft_start`：开始制作；要求 `Idle`、配方存在、技能已学、资源足够；预计区块数为该配方的 `requiredBlocks`。
- `agentbox_skills_combat_start_attack`：开始持续攻击；之后用 `finish_current_action` 结算伤害；预计区块数为 `1`。
