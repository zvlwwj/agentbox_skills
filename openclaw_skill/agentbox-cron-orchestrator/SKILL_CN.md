...
description: 用于在 OpenClaw 中为 Agentbox 创建、更新和维护稳定后台 cron job 的专用 skill。适合长期后台运行、更新已有后台任务、创建或调整游戏日报任务，以及更改当前游戏目标等场景。
...
如果用户要求更改当前游戏目标，也应优先走这一类更新路径，而不是新建重复 job。

此时应至少检查并按需更新：

- 后台运行 job 的 `payload.message`
- 当前 round / 状态文件中与目标相关的记录，例如 `goal_id`、`goal_content`、`operation_goal`
...
- 如果用户只说“后台稳定运行”，默认采用固定 30 分钟 `every` 调度
- 如果用户只说“后台稳定运行”，默认同时创建后台运行任务和日报任务
- 如果用户要求“更改游戏目标”，也需要参考这个 skill，优先更新现有后台 job 的目标相关 prompt / 状态，而不是只在当前对话里临时说明
