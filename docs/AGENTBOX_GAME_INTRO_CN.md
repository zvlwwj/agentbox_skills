...
在当前实现里，传送完成还有一个非常重要的副作用：

- 如果目标地块上有 `groundTokens`，`teleport.finish` 会自动尝试拾取这些 token

这意味着：

- “传送到代币地块” 的真正收益动作，往往发生在 `finish`
- 不一定需要一个单独的“手动拾取代币”动作
- 自动拾取成功后，拿到的 AGC 仍可能先进入 `unreliableBalance`，而不是直接成为 `reliableBalance`
...
- `unreliableBalance` 是已经记到角色钱包名下、但尚未完成稳定化的余额
...
- 地块上的 `groundTokens` 在被角色获取前，仍属于地图地块状态
- 它们被拿到后，不一定立刻变成 `reliableBalance`
- 在当前实现里，很多情况下它们会先进入角色的 `unreliableBalance`
...
- `chain` 更接近链上即时事实，但可直接拿到的字段较少
- `indexer` 更适合聚合展示与附近信息检索，但可能存在短暂同步延迟
- `auto` 会在体验上更完整，但可能混合不同来源的数据

因此：

- 动态信息不是天然“绝对同时一致”的
- 某些时刻可能出现“链上状态已推进，但 indexer 还显示旧状态”
- 在执行敏感链上动作前，应优先参考最新链上事实，而不是只依赖历史摘要或单一 indexer 结果
